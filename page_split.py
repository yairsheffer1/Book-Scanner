...
def page_split(image_path, num=""):
    ...
    # Filter minima to be at least 100 pixels apart
    valid_minima = filter_minima_by_distance(minima_indices, projection, min_distance=300)

    # Select the middle one **by index order**
    # if len(valid_minima) == 3:
    #     middle = valid_minima[1]  # Select the second one (middle by index order)
    # elif len(valid_minima) > 0:
    #     middle = valid_minima[-1]  # Fallback: use the last available one
    # else:
    #     middle = np.argmin(projection)  # Ultimate fallback
    ...
    left_page = image_colored[:, :middle + 20]
    right_page = image_colored[:, middle - 20:]


    # Save both pages
    file_path_left = os.path.join("selected_images", f"left_page_{num}")
    file_path_right = os.path.join("selected_images", f"right_page_{num}")
...
# Call the function
page_split(r"C:\Users\user\PycharmProjects\pythonProject10\selected_images\selected_image_100.png")
